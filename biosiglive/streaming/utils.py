...
def dic_merger(dic_to_merge: dict, new_dic: dict = None) -> dict:
    ...
    if not new_dic:
        ...
        for key in dic_to_merge.keys():
            if isinstance(dic_to_merge[key], tuple):
                dic_to_merge[key] = list(dic_to_merge[key])
            if isinstance(new_dic[key], tuple):
                new_dic[key] = list(new_dic[key])
            if dic_to_merge[key] is None:
                dic_to_merge[key] = [dic_to_merge[key]]
            if new_dic[key] is None:
                new_dic[key] = [new_dic[key]]
            if isinstance(new_dic[key], (int, float, np.int32)):
                new_dic[key] = [new_dic[key]]
            if isinstance(dic_to_merge[key], (int, float, np.int32)):
                dic_to_merge[key] = [dic_to_merge[key]]
